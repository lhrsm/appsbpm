...
BEGIN
    ...
    -- 1. Localizar o vínculo do usuário
    ...
    IF v_link_id IS NULL THEN
        -- Tentar localizar por e-mail se não houver vínculo por user_id
        SELECT id, cpf_reference, associado_id, dependente_id, person_type
        INTO v_link_id, v_cpf_ref, v_assoc_id, v_dep_id, v_person_type
        FROM public.external_account_links
    ...
    -- 2. Validar dados institucionais
    IF v_assoc_id IS NULL AND v_dep_id IS NULL THEN
        -- Tentar localizar associado pelo CPF
    ...
    -- 4. Registrar Auditoria (tabela externa simplificada para evitar falhas)
    -- INSERT INTO public.external_auth_audit_logs... (omitido para evitar dependência de tabela inexistente)
